...
const GAME_ICONS: Record<string, string> = {
  "One Piece TCG": "🏴‍☠️", "Pokémon TCG": "⚡", "Magic: The Gathering": "✨",
  "Yu-Gi-Oh!": "👁️", "Flesh & Blood": "⚔️", "Digimon": "🎭",
  "Dragon Ball": "🐉", "Lorcana": "🌟", "Weiss Schwarz": "🎌", "Union Arena": "🎮",
};
...
cons
cat > ~/Desktop/suitcg/app/sell/page.tsx << 'EOF'
"use client";
import { useState, useEffect } from "react";
import { supabase } from "../../lib/supabase";
...
export default function Sell() {
  ...
  const [form, setForm] = useState({
    ...
    condition: "NM", price_usd: "", quantity: "1",
  ...
  useEffect(() => {
    const addr = typeof window !== "undefined" ?
      (localStorage.getItem("wavetcg_wallet_address") || localStorage.getItem("connected_wallet") || "") : "";
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#000008" }}>
      ...
      <div style={{ maxWidth: "640px", margin: "0 auto", padding: "20px clamp(12px, 3vw, 24px)" }}>
        ...
        {/* Safety tip */}
        <div style={{ background: "rgba(255,180,0,0.05)", border: "1px solid rgba(255,180,0,0.2)", borderRadius: "10px", padding: "10px 14px", marginBottom: "16px", display: "flex", gap: "8px", alignItems: "flex-start" }}>
          <span style={{ fontSize: "14px", flexShrink: 0 }}>🛡️</span>
          <p style={{ fontSize: "11px", color: "#c8a84b", lineHeight: 1.5, margin: 0 }}>
            <strong>Safety Tip:</strong> Connect at least 2 social accounts to build trust with buyers.
          </p>
        </div>
        ...
        {!walletAddress ? (
          <div style={{ background: "#050515", border: "1px solid rgba(0,153,255,0.15)", borderRadius: "16px", padding: "48px 24px", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "13px", color: "#8899bb", marginBottom: "20px" }}>You need to connect your Sui wallet to list cards.</p>
          </div>
        ...
        /* Verification gate */
          ...
          <div style={{ background: "#050515", border: "1px solid rgba(255,153,0,0.2)", borderRadius: "16px", padding: "clamp(20px, 4vw, 40px)", textAlign: "center" }}>
            ...
            <p style={{ fontSize: "13px", color: "#8899bb", marginBottom: "20px", lineHeight: 1.6 }}>
              Connect at least <strong style={{ color: "#0099ff" }}>2 social accounts</strong> to list cards. This builds trust in our marketplace.
            </p>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "24px", textAlign: "left" }}>
              {[
                { label: "Twitter / X", icon: "𝕏", connected: profile?.twitter },
                { label: "Discord", icon: "💬", connected: profile?.discord },
                { label: "Telegram", icon: "✈️", connected: profile?.telegram },
              ].map((s, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", background: s.connected ? "rgba(0,255,100,0.04)" : "#0a1628", border: `1px solid ${s.connected ? "rgba(0,255,100,0.15)" : "rgba(255,255,255,0.06)"}`, borderRadius: "10px" }}>
                  ...
                  <span style={{ fontSize: "11px", color: s.connected ? "#00ff88" : "#ff6b6b", fontWeight: 600 }}>
            ...
            <div style={{ fontSize: "13px", color: "#8899bb", marginBottom: "16px" }}>
              <strong style={{ color: verifiedCount >= 2 ? "#00ff88" : "#ff9955" }}>{verifiedCount}/2</strong> accounts connected
            </div>
            <a href={`/profile/${walletAddress}`} style={{ display: "block", background: "linear-gradient(135deg, #0055ff, #0099ff)", color: "#fff", padding: "12px 24px", borderRadius: "10px", fontSize: "13px", fontWeight: 600, textDecoration: "none" }}>
        ...
        /* Success */
          ...
          <div style={{ background: "#050515", border: "1px solid rgba(0,255,100,0.2)", borderRadius: "16px", padding: "clamp(32px, 6vw, 60px)", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: "10px", justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <button onClick={resetForm} style={{ background: "transparent", color: "#c8d8f0", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", padding: "11px 24px", fontSize: "13px", cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}>
                List Another Card
              </button>
            </div>
          </div>
          ...
          <div style={{ background: "#050515", border: "1px solid rgba(0,153,255,0.15)", borderRadius: "16px", padding: "clamp(16px, 4vw, 32px)" }}>
            ...
            <div style={{ marginBottom: "24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                {["Card Details", "Pricing", "Review"].map((label, i) => (
                  <div key={i} style={{ fontSize: "10px", color: step > i + 1 ? "#00ff88" : step === i + 1 ? "#0099ff" : "#444460", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: step === i + 1 ? 600 : 400 }}>
                    {step > i + 1 ? "✓ " : `${i + 1}. `}{label}
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", gap: "4px" }}>
                {[1, 2, 3].map(s => (
                  <div key={s} style={{ flex: 1, height: "3px", borderRadius: "2px", background: step >= s ? "linear-gradient(90deg, #0055ff, #0099ff)" : "rgba(255,255,255,0.08)", transition: "background 0.3s" }} />
                ))}
              </div>
            </div>
            ...
            {step === 1 && (
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                <div style={{ fontFamily: "Cinzel, serif", fontSize: "15px", color: "#ffffff", marginBottom: "4px" }}>Card Details</div>

                {/* Game selector — grid of pills */}
                <div>
                  <label style={labelStyle}>Game *</label>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(130px, 1fr))", gap: "6px" }}>
                    {GAMES.map(g => (
                      <button key={g} onClick={() => setForm(p => ({ ...p, game: g }))}
                        style={{ padding: "8px 10px", borderRadius: "8px", cursor: "pointer", fontFamily: "DM Sans, sans-serif", fontSize: "11px", fontWeight: 500, border: form.game === g ? "1px solid #0099ff" : "1px solid rgba(255,255,255,0.08)", background: form.game === g ? "rgba(0,153,255,0.12)" : "transparent", color: form.game === g ? "#0099ff" : "#8899bb", textAlign: "left", display: "flex", alignItems: "center", gap: "6px" }}>
                        <span>{GAME_ICONS[g] || "🃏"}</span>
                        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.split(":")[0].split(" ").slice(0, 2).join(" ")}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ position: "relative" }}>
                  ...
                  <input value={form.name}
                    ...
                    placeholder={`Search ${form.game} cards...`}
                    style={inputStyle} />
                    ...
                    <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#050515", border: "1px solid rgba(0,153,255,0.2)", borderRadius: "10px", zIndex: 100, marginTop: "4px", boxShadow: "0 8px 32px rgba(0,0,0,0.6)", overflow: "hidden" }}>
                      {cardSuggestions.map((card, i) => (
                        <div key={i} onClick={() => { setForm(p => ({ ...p, name: card.name, set_name: card.set || p.set_name, card_number: card.number || p.card_number })); setShowSuggestions(false); }}
                          style={{ padding: "10px 14px", cursor: "pointer", borderBottom: i < cardSuggestions.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none" }}
                ...
                <div>
                  ...
                  {form.image_url && (
                    <div style={{ marginTop: "8px", width: "60px", height: "80px", borderRadius: "6px", overflow: "hidden", border: "1px solid rgba(0,153,255,0.2)" }}>
                      <img src={form.image_url} alt="preview" style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                    </div>
                  )}
                </div>

                {error && <div style={{ fontSize: "12px", color: "#ff6b6b", padding: "8px 12px", background: "rgba(255,50,50,0.06)", borderRadius: "6px", border: "1px solid rgba(255,50,50,0.15)" }}>{error}</div>}

                <button onClick={() => { if (!form.name.trim()) { setError("Please enter a card name"); return; } setError(""); setStep(2); }}
                  style={{ width: "100%", background: "linear-gradient(135deg, #0055ff, #0099ff)", color: "#fff", border: "none", borderRadius: "8px", padding: "13px", fontSize: "14px", fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif", marginTop: "4px" }}>
                  Next → Pricing
                </button>
              </div>
            )}
            ...
            {step === 2 && (
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                <div style={{ fontFamily: "Cinzel, serif", fontSize: "15px", color: "#ffffff", marginBottom: "4px" }}>Condition & Pricing</div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                  <div>
                    ...
                    <div style={{ position: "relative" }}>
                      ...
                      <input value={form.price_usd} onChange={e => setForm(p => ({ ...p, price_usd: e.target.value }))} placeholder="0.00" type="number" min="0"
                ...
                {/* Fee info */}
                <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", background: "#0a1628", borderRadius: "8px", border: "1px solid rgba(0,153,255,0.08)" }}>
                  <span style={{ fontSize: "12px", color: "#8899bb" }}>Platform fee</span>
                  <span style={{ fontSize: "12px", color: "#0099ff" }}>1% on sale</span>
                </div>
                {form.price_usd && parseFloat(form.price_usd) > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", background: "rgba(0,255,136,0.04)", borderRadius: "8px", border: "1px solid rgba(0,255,136,0.1)" }}>
                    <span style={{ fontSize: "12px", color: "#8899bb" }}>You receive</span>
                    <span style={{ fontSize: "13px", color: "#00ff88", fontWeight: 600 }}>${(parseFloat(form.price_usd) * 0.99).toFixed(2)}</span>
                  </div>
                )}
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  <button onClick={() => { if (!form.price_usd || parseFloat(form.price_usd) <= 0) { setError("Please enter a valid price"); return; } setError(""); setStep(3); }}
                    style={{ flex: 2, background: "linear-gradient(135deg, #0055ff, #0099ff)", color: "#fff", border: "none", borderRadius: "8px", padding: "13px", fontSize: "14px", fontWeight: 600, cursor: "pointer", fontFamily: "DM Sans, sans-serif" }}>
                    Next → Review
                  </button>
                </div>
              </div>
            ...
            {step === 3 && (
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                <div style={{ fontFamily: "Cinzel, serif", fontSize: "15px", color: "#ffffff", marginBottom: "4px" }}>Review & List</div>
                ...
                <div style={{ background: "#0a1628", border: "1px solid rgba(0,153,255,0.15)", borderRadius: "12px", padding: "16px", display: "flex", gap: "14px", alignItems: "flex-start" }}>
                  {form.image_url && (
                    <div style={{ width: "60px", height: "80px", borderRadius: "6px", overflow: "hidden", flexShrink: 0 }}>
                      <img src={form.image_url} alt={form.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} onError={e => { (e.currentTarget as HTMLImageElement).style.display = "none"; }} />
                    </div>
                  )}
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                      <span style={{ fontSize: "11px", color: CONDITION_COLORS[form.condition], padding: "2px 8px", background: `${CONDITION_COLORS[form.condition]}12`, borderRadius: "4px", border: `1px solid ${CONDITION_COLORS[form.condition]}30` }}>{form.condition}</span>
                ...
                <div style={{ background: "#0a1628", borderRadius: "10px", padding: "14px", display: "flex", flexDirection: "column", gap: "10px" }}>
                  {[
                    ["Listing price", `$${parseFloat(form.price_usd || "0").toFixed(2)}`],
                    ["Platform fee (1%)", `-$${(parseFloat(form.price_usd || "0") * 0.01).toFixed(2)}`],
                    ["You receive", `$${(parseFloat(form.price_usd || "0") * 0.99).toFixed(2)}`],
                  ].map(([label, val], i) => (
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  ...
                  <button onClick={handleSubmit} disabled={loading}
                    style={{ flex: 2, background: loading ? "rgba(0,153,255,0.3)" : "linear-gradient(135deg, #0055ff, #0099ff, #00ffcc)", color: "#fff", border: "none", borderRadius: "8px", padding: "13px", fontSize: "14px", fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", fontFamily: "DM Sans, sans-serif" }}>
